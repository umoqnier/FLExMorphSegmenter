...
\Diego{Descripcion categorizada de los hiperparámetros}
...
En este capítulo se detallará el desempeño de los modelos que fueron mencionados en el capítulo \ref{chap:metodology} y se profundizará en los resultados obtenidos. Además, se examinará el método de evaluación utilizado para los modelos generados.
...
% Cual salio mejor
Con base en la arquitectura propuesta observamos que el modelo que mejor precisión tiene es justamente el \textbf{\textsf{linearCRF\_l2\_zero}} (Ver tabla \ref{tab:models-comparation}), dónde se codifica toda la información disponible de la lengua en las \textit{feature functions}, que se mencionan en \ref{feature_functions_descr}. Además, los hiperparametros \textit{Elastic Net} del algoritmo de aprendizaje, configurados en $L_1 = 0.1$ y $L_2 = 0$, resultan relevantes dado que son lo únicos que varían respecto a los otros modelos del mismo entorno de experimentación (Ver tabla \ref{tab:models-full-featured}). 

Vale la pena mencionar que, a pesar de la variación de hiperparametros, la precisión de la familia de modelos \textsf{linearCRF}, presentes en la tabla \ref{tab:models-full-featured}, presentan una variación en la precisión poco representativa (menos de $0.001$ de precisión entre modelos). Entonces, parece ser que la información codificada en las \textit{feature functions} son las que determinan, en gran medida, el desempeños de los \textit{CRFs} para la correcta generación de \textit{BIO Labels} (glosa) para el idioma otomí. 


\Diego{Poner grafica de la función de perdida y comentar}
